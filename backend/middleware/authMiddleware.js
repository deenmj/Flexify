...
export const requireRentVerified = (req, res, next) => {
  if (!req.user) return res.status(401).json({ message: "Not authorized" });
  const isVerified = req.user.rentVerificationStatus === "approved" || req.user.verificationStatus === "approved";
  const isAdmin = ["staff", "subadmin", "admin", "superadmin"].includes(req.user.role);
  if (!isVerified && !isAdmin) {
    return res.status(403).json({ message: "Rent verification required to publish rentals." });
  }
...
export const requireSalesVerified = (req, res, next) => {
  if (!req.user) return res.status(401).json({ message: "Not authorized" });
  const hasAccess = req.user.hasSalesAccess === true || req.user.verificationStatus === "approved";
  const isAdmin = ["staff", "subadmin", "admin", "superadmin"].includes(req.user.role);
  if (!hasAccess && !isAdmin) {
    return res.status(403).json({ message: "Sales dashboard access required to publish sales." });
  }
  next();
};

...
export interface VehicleSaleListing {
  _id: string;
  seller: User | string;
  listingType: 'SALE';
  title: string;
  price: number;
  condition: 'Brand New' | 'Excellent' | 'Good' | 'Used';
  mileage: number;
  city: string;
  contactPhone: string;
  description?: string;
...
export const getSaleListingSlug = (listing: VehicleSaleListing) => {
  const base = `${listing.title}-${listing.city}`.toLowerCase();
  const slug = base.replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '');
  return `${slug}--${listing._id}`;
};
...
export const saleListingApi = {
  ...
  getAll: (params?: Record<string, string>) => {
    const query = params ? '?' + new URLSearchParams(params).toString() : '';
    return apiFetch<{ listings: VehicleSaleListing[]; pagination: any }>(`/sale-listings${query}`);
  },

  getById: (id: string) => apiFetch<VehicleSaleListing>(`/sale-listings/${id}`),

  // Authenticated
  getMy: () => apiFetch<VehicleSaleListing[]>('/sale-listings/my'),

  create: (formData: FormData) =>
    fetch(`${API_BASE_URL}/sale-listings`, {
      method: 'POST',
      headers: authHeadersOnly(),
      body: formData,
    }).then(async (res) => {
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Failed to create sale listing');
      return data as VehicleSaleListing;
    }),

  markAsSold: (id: string) =>
    apiFetch<{ message: string; listing: VehicleSaleListing }>(`/sale-listings/${id}/sold`, {
      method: 'PATCH',
    }),

  delete: (id: string) =>
    apiFetch<{ message: string }>(`/sale-listings/${id}`, { method: 'DELETE' }),

  // SubAdmin+ 
  getPending: () => apiFetch<VehicleSaleListing[]>('/sale-listings/pending'),

  approve: (id: string) =>
    apiFetch<{ message: string; listing: VehicleSaleListing }>(`/sale-listings/${id}/approve`, {
      method: 'PATCH',
    }),

  reject: (id: string, reason: string, comment?: string) =>
    apiFetch<{ message: string; listing: VehicleSaleListing }>(`/sale-listings/${id}/reject`, {
      method: 'PATCH',
      body: JSON.stringify({ reason, comment }),
    }),

  // Admin: all listings
  getAdminAll: () => apiFetch<VehicleSaleListing[]>('/sale-listings/all'),
};

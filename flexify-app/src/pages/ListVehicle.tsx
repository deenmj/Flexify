...
import { Select as AntSelect, message, Row, Col, Space } from 'antd';
import { Car, MapPin, Settings, Image, Locate, Gauge, Fuel, Users, Zap, Shield, FileText } from 'lucide-react';
...
export default function ListVehicle() {
  ...
  useEffect(() => {
    const fetchMakes = async () => {
      ...
      try {
        ...
        setMakes(data);
      } catch (err) { console.error("Makes fail", err); }
      ...
      navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        try {
          ...
          if (data && data.address) {
            ...
            message.success({ content: `Location Detected: ${city || 'Available'}`, key: 'locate' });
          }
        } catch (err) { message.success({ content: 'Location Found', key: 'locate' }); }
      }, () => message.error({ content: 'Location Access Denied', key: 'locate' }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      window.scrollTo(0, 0);
      // Reset form omitted for space, would be here
    ...
    <div className="list-vehicle-page">
      <section className="static-hero">
        <div className="container">
          ...
          <p>Join our premium marketplace and start earning today</p>
        </div>
      </section>
                  ...
                  <div className="form-card-body">
                    ...
                    <Row gutter={[16, 16]}>
                      <Col span={12}>
                        <div className="input-group">
                          <label>Make</label>
                          <AntSelect showSearch className="antd-select-large" placeholder="Select Brand" value={selectedMake || undefined} onChange={(val) => { setSelectedMake(val); setSelectedModel(''); }} loading={loadingMakes}>
                      ...
                      <Col span={12}>
                        <div className="input-group">
                          ...
                          <select className="input-field" value={form.seats} onChange={(e) => setForm({ ...form, seats: e.target.value })}>
                            <option value="2">2</option><option value="4">4</option><option value="5">5</option><option value="7">7</option><option value="8">8+</option>
                          </select>
                        </div>
                      </Col>
                    </Row>
              ...
              <Col xs={24}>
                <div className="form-card" style={{ animationDelay: '0.2s' }}>
                  <div className="form-card-header">
                    ...
                    <button type="button" onClick={handleGetLocation} className="btn-locate-premium"><Locate size={16} /> Auto-Detect</button>
                  </div>
                  <div className="form-card-body">
                    <Row gutter={[24, 24]}>
                      <Col xs={24} md={8}>
                      ...
                      <Col xs={24} md={8}>
                        <div className="input-group"><label>City / Town</label><input className="input-field" placeholder="Enter City" value={form.city} onChange={(e) => setForm({ ...form, city: e.target.value })} required /></div>
                      </Col>
                      
                      <Col xs={24} md={12}>
                        <div className="input-group">
                          <label>Listing Description</label>
                          <textarea className="input-field" rows={5} placeholder="Describe your vehicle's condition, rules, and unique features..." value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} required />
                        </div>
                      </Col>

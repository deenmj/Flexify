...
export default function EditVehicle() {
    ...
    const fetchData = async () => {
      try {
        ...
        setForm({
          ...
          lat: (vehicle.location?.coordinates?.[1] || '').toString(),
          lng: (vehicle.location?.coordinates?.[0] || '').toString(),
      ...
      navigator.geolocation.getCurrentPosition(async (pos) => {
        ...
        try {
          ...
          if (data && data.address) {
            ...
            message.success({ content: `Location Detected: ${city}`, key: 'locate' });
          }
        } catch (err) { message.success({ content: 'Location Found', key: 'locate' }); }
      }, () => message.error({ content: 'Access Denied', key: 'locate' }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate('/dashboard');
    } catch (err: any) { setError(err.message || 'Error occurred while saving'); }
    finally { setSaveLoading(false); }
  };

  if (loading) return <div className="container" style={{ padding: '80px 1rem', textAlign: 'center' }}>Updating local cache...</div>;
    ...
    <div className="list-vehicle-page">
      <section className="static-hero">
        <div className="container">
          ...
          <p>Keep your details fresh and accurate for renters</p>
        </div>
      </section>
      ...
      <section className="list-vehicle-wrapper">
        <form onSubmit={handleSubmit}>
              ...
              <div className="form-card">
                ...
                <div className="form-card-body">
                  <Row gutter={[16, 16]}>
                    ...
                    <Col span={12}><div className="input-group"><label>Capacity</label><input className="input-field" value={form.engineCapacity} onChange={(e) => setForm({ ...form, engineCapacity: e.target.value })} /></div></Col>
                  ...
                  <div className="features-group">
                    <label className="input-group label" style={{ marginBottom: '1rem' }}>Amenities</label>
                    <div className="features-grid">
                      {VEHICLE_FEATURES.map(f => (
                        <div key={f.id} className={`feature-item ${form.features.includes(f.id) ? 'active' : ''}`} onClick={() => handleFeatureToggle(f.id)}>
                          <span className="feature-icon">{f.icon}</span><span className="feature-label">{f.label}</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          <button type="submit" className="submit-vehicle-btn" disabled={saveLoading}>
